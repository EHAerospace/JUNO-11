...
//is the sensor connected?
  if (event.pressure)
  {
   ...
   //was this reading higher than last reading??
    if (alti > maxalti) {
      maxalti = alti;
    } else {
    ...
    //convert float to byte to transmit via i2c
    alticonvert = (byte) maxalti;
    tempconvert = (byte) maxtemp;

    //send data via i2c pins
    Wire.beginTransmission(8); // transmit to device #8
    Wire.write(alticonvert);
    Wire.write("and");
    Wire.write(tempconvert);
